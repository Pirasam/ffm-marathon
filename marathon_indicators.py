...
(function(){
  ...
  // Perzentil statt striktem Min/Max fuer die Achsen-Grenzen - ein einzelner
  // untypischer Ausreisser (z.B. ein sehr langsamer Reha-Lauf) soll nicht die
  // ganze Skala auseinanderziehen und den eigentlich interessanten Bereich
  // unlesbar zusammenquetschen.
  ...
  function renderChart(canvas,rawPts,trendPts,opts){
    ...
    // Trend IMMER voll sichtbar (Kerninformation); rohe Streupunkte nur bis
    // P5/P95 - ein einzelner extremer Ausreisser darf ausserhalb liegen.
    var rawVals=(rawPts||[]).map(function(p){return p.v;});
    var trendVals=(trendPts||[]).map(function(p){return p.v;});
    var lo=trendVals.length?Math.min.apply(0,trendVals):Infinity;
    var hi=trendVals.length?Math.max.apply(0,trendVals):-Infinity;
    if(rawVals.length){
      lo=Math.min(lo,percentile(rawVals,0.05));
      hi=Math.max(hi,percentile(rawVals,0.95));
    }
